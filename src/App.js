...
const useCountry = (name) => {
  ...
  useEffect(() => {
    axios.get(`https://studies.cs.helsinki.fi/restcountries/api/name/${name}`).then(data => setCountry({...data, found:true})).catch(() => {if(name === ''){setCountry(null)} else{setCountry({found:false})}})
  }, [name])
  return country
}
